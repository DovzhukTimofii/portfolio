    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className="w-full flex justify-between items-center max-w-7x1 mx-auto">
        <Link to="/" className="flex items-center gap-2" onClick={() => { setActive(""); window.scrollTo(0, 0); }}>
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer sm:flex">Timofii</p>
        </Link>
        <div className="flex flex-row justify-between items-end w-1/2 2xl:w-1/4 xl:1/3">
          <div className="flex flex-row">
            <div className="w-[28px] h-[23px] flex justify-center border-r-white border-r "><Link to={'/portfolio/'}><span className="hover:text-secondary" >EN</span></Link></div>
            <div className="w-[28px] h-[23px] flex justify-center "><span className="text-secondary">UA</span></div>
          </div>
