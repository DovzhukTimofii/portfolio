...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    emailjs.send("service_inah4es", "template_nq2wpcm", {
      form_name: form.name,
      to_name: 'Timofii',
      from_email: form.email,
      to_email: 'dovzhuktima@gmail.com',
    ...
    "xLhFC6LqkPp-1JTs-"
      ...
      () => {
        setLoading(false);
        alert("Дякую. Я зв'яжуся з вами якнайшвидше.");
    ...
    >
      ...
      >
        ...
        >
          ...
          <label className='flex flex-col'>
            <span className='text-white font-medium mb-4'>Ваша електронна пошта</span>
            <input
              type='email'
              name='email'
              value={form.email}
              onChange={handleChange}
              placeholder="Яка ваша веб-адреса?"
              className='bg-tertiary py-4 px-6 placeholder:text-secondary text-white rounded-lg outline-none border-none font-medium'
            />
          </label>
          <label className='flex flex-col'>
            ...
            <textarea
              ...
              value={form.message}
              onChange={handleChange}
              placeholder='Що ти хочеш сказати?'
              className='bg-tertiary py-4 px-6 placeholder:text-secondary text-white rounded-lg outline-none border-none font-medium'
            />
          </label>
